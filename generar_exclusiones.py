...
# -*- coding: utf-8 -*-
"""
Script to generate an exclusion file for already-processed JRVs
Reads the JSON with Drive URLs and generates a Set of JRVs to exclude in the browser
"""
...
def main():
    ...
    # Filter JRVs that ALREADY HAVE url_drive
    jrvs_con_url = set()
    jrvs_sin_url = []
    ...
    try:
        with open(OUTPUT_JS, 'w', encoding='utf-8') as f:
            f.write("// Auto-generated by generar_exclusiones.py\n")
            f.write("// JRVs that ALREADY HAVE url_drive and must be EXCLUDED from processing\n")
            f.write(f"// Total JRVs to exclude: {con_url:,} ({porcentaje_completo:.1f}%)\n")
            f.write(f"// Generated: {import_datetime()}\n\n")

            f.write("const JRVS_A_EXCLUIR = new Set([\n")

            # Write JRVs in blocks of 20 per line (more compact)
            ...
            f.write("// Helper function to check whether a JRV should be processed\n")
    ...
    # Also generate a human-readable log of missing JRVs
    if jrvs_sin_url:
...
def import_datetime():
    """Helper to import datetime without moving the import to the top"""
